...
**An EMPTY library search says where to look next, and says it harder the second time.** `{"tracks":[]}`
is a true answer and a useless one: it reports that the station does not own the record and nothing
about the tool that can reach it. Measured on a `artists like mitch murder` refill against a library
of rock and metal — the model found the artist at a provider, asked `similar_artists` for its
neighbours, then searched the LIBRARY for each neighbour in turn, got nothing every time, and ran out
of tool steps before it ever answered; twenty-four records came from the brief-blind floor and
nothing in the log connected the two facts. So an empty result carries a `note`, and the note
ESCALATES: once is a fact about one query and the suggestion is to reach past it,
`EMPTY_RUN_BEFORE_STOP` in a row is a fact about the LIBRARY and the note stops suggesting and starts
telling. That second step is a withdrawal rather than a stronger preference, because preferring the
library is exactly what the model was doing when it searched it for the fourth artist running. The
count is per instance and the tool is scoped per invocation, so it describes THIS generation, and a
hit resets it — what is counted is a RUN of misses rather than a tally of them. What it deliberately
does not do is fold provider results in: that would collapse the HAS versus CAN GET split above into
a list the model can no longer tell apart.
