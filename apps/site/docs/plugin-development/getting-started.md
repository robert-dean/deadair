...
In the console, open **Settings → Plugins** and press **Rescan**. Your plugin appears as a card marked **Installed**, in the Discovered state, which means the station loaded it and has not been told to use it. If it appears as Failed instead, its card and its page say why, including the directory the station read.
...
## Change it, and restart to see the change

Rebuild after every change, and copy `dist/` back. Then **restart the station**. Reload and Rescan both re-run the plugin's setup with its current settings, but neither loads new code: Node keeps a module it has imported for as long as the process runs, so a changed plugin is only read again by a new process.

The same goes for a new version of somebody else's plugin: replace its files and restart. Removing a plugin is deleting its folder and pressing Rescan.
