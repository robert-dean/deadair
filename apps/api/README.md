...
| **Languages**   | [modules/languages](src/modules/languages)       | The console language packs an admin imported, one per language, in `deadair.console_languages`: every word the console says, translated, stored as the pack's catalog verbatim. The API checks only the header and the catalog's shape; whether each string fits is decided by the console that loads it, against the English it was built with. The console's language only, never `stream.language`. |
...
| **Playlists**   | [modules/playlists](src/modules/playlists)       | Two kinds of playlist. A provider's is read live: every catalog-capable plugin's playlists, aggregated, plus one plugin's playlist tracks on demand, each a call through `PluginInvoker`, with a failing plugin degrading to a `CatalogSourceError` entry rather than failing the request. The station's OWN playlists are rows in `deadair.playlists`, cloned in by an import (preview, then import, through one planner) and written out as a file; a record the library does not hold is kept as a placeholder in its place. |
...
| **Almanac**     | [modules/almanac](src/modules/almanac)           | What happened on the station's own date, out of whatever almanac plugins are installed. Weather's sibling with the perishability taken out: it answers about a DATE rather than a place, the first plugin that answers wins, and what a station reaches for first (`rotation.almanacLean`) is decided here so the tool and the break writers cannot lean differently. The day is resolved in `station.timezone` against the moment a break AIRS. It owns no subjects and no loop. |
| **Podcasts**    | [modules/podcasts](src/modules/podcasts)         | Somebody else's programmes, out of whatever podcast plugins are installed, and the station's own record of the episodes it knows about: what the feed said, whether the station fetched the audio, whether it aired. A list of subscriptions rather than a merge, like News. Owns no loop: the `podcasts.refresh` cron re-reads the feeds, and a refresh is an upsert that never touches what the station did with an episode. |
| **Narrations**  | [modules/narrations](src/modules/narrations)     | What the station reads out in its own voice (a book by the chapter, a column by the issue), out of whatever narration plugins are installed, and its record of which pieces it has spoken and aired. `news`'s sibling rather than `podcast`'s: the plugin hands over WORDS and the station makes the audio. A piece becomes a production opened already written, and this module's ripen pass is also where a finished one is attached to the piece that asked for it. |
...
| `mcp` (hand-written, in `src/mcp`) | `POST /mcp`, the MCP endpoint over Streamable HTTP, stateless (`GET` answers 405). Guarded by `oauth.grant`: an app's OAuth grant for this resource, never a console session. Its tools are generated beside it from operations a contract flags `mcp`, plus the hand-written ones there; see `docs/internals/authentication.md` |
...
| `art.breaks`              | `GET /art/breaks`, `POST /art/breaks/:kind` (multipart), `DELETE /art/breaks/:kind`: the picture a kind of break wears, and putting the shipped one back. Registered BEFORE `art` in `routes.setup.ts`, because `/art/breaks` also matches `/art/:id` and Koa takes the first router that matches |
...
| `podcasts`                | `GET /podcasts/shows`, `GET /podcasts/episodes`, and `POST /podcasts/refresh`, which queues a refresh rather than doing one on the request |
| `playlists`               | `GET /playlists`, `GET /playlists/:pluginId/:playlistId/tracks` |
| `station.playlists`       | The station's own playlists: `GET /station-playlists`, `GET`/`PATCH`/`DELETE /station-playlists/:id`, `GET /station-playlists/:id/export` (a file another station can import), `POST /station-playlists/:id/fill` (queue a look-up of the records the library lacks), and `POST /station-playlists/import/preview` and `/import`, which run one planner so the preview is the decision |
| `playout`                 | `GET /playout/status`, `POST /playout/playlist`, `/playout/station-playlist`, `/playout/chart`, `/playout/skip`, `/playout/start`, `/playout/stop` (stand the station down: out of service, not merely paused). `GET /playout/audio/:sourceId` is how the player fetches every record — one URL on this machine whether or not the bytes are here yet — and is gated by a URL signed with the bridge secret. Internal and secret-gated by the `/playout/bridge/` prefix, never in the SDK: `POST /playout/bridge/aired` (Liquidsoap's air confirmation) and `POST /playout/bridge/starve`. There is no listener hook — the audience is read from the event feed and the stats poll instead; see the audience gate in `stream/README.md` |
...
| `director`                | `GET`/`POST`/`PATCH /director/air` (`POST` builds a broadcast from a playlist read at that moment, a chart, or a playlist the station owns; `PATCH` sets what puts the station on air, `audience` or `always`), `GET /director/air/order`, `PUT /director/air/persona`, `POST /director/air/extend`, `/replan`, `/shuffle`, `/segments`, `/tracks`, `PATCH`/`DELETE /director/air/hold`, and `PATCH`/`DELETE /director/air/items/:itemId` |
