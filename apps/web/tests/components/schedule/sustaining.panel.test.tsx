// What plays between blocks is five station settings, and this panel is the only place they are
// edited now. Two things about that are invisible from the API side and are what is pinned here: a
// save sends exactly those five keys and nothing else on the settings page, and a field the operator
...
describe('SustainingPanel', () => {
    ...
    it('saves exactly the five keys it owns', async () => {
    ...
    it('drops both halves of the source together when the picker is cleared', async () => {
        ...
        await user.click(screen.getByLabelText('Play from no playlist'));
        await user.click(screen.getByRole('button', { name: 'Save' }));
