...
**Check-up** is at the foot of the navigation, and has five tabs. **Machinery** shows the verdict with its sentence and remedy, the listener count, whether the stream is up, what is queued and which mounts are published, then **Needs you**, the loops, the plugins, how many records are held, local and measured, the disk and the build. **What it has been doing** is the activity feed, filterable by part and severity. **What it cost** is every call the station made and what it spent. **Logs** is the station's own log and the audio chain's. **What's new** is what changed in each release. Under the mounts there is a `deadair://` link and a code to scan, either of which hands a [listener app](./listening.md#on-android) this station's address without anybody typing it. On the **Desk**, **Why is it not on air?** opens every gate with the switch for what puts the station on air. The same switch, and the list of refused players, are under **Settings**, **Playout** and **Settings**, **Stream**.

## Which release, and what is new

**What's new** lists every release the station contains, newest first, with what changed in each, and marks the one it is running. The notes are the station's own changelog, built into it, so the page works on a station with no internet connection. The **Build** section of **Machinery** links to it.

The station also checks whether a newer release is out. When one is, the console's header names it, **What's new** lists it first with its notes and a link to its page, and the Build section says so. The station asks GitHub for the project's public list of releases once when it starts and then every six hours, one anonymous request each time. **Check now**, on **What's new**, asks straight away, for an operator who has just heard a release went out; a second press within a minute gets the same answer rather than a second request. It sends nothing about the station: no name, no version and nothing about who is listening. GitHub sees the request arrive from your network's address, as it would any visit. The station never installs anything itself; upgrading is still pulling the new image, as you installed it.

A station following `latest` is compared by the newest release it contains rather than by a version number, which only a release build carries. So it is told about a release only once one is out that it has not already picked up from `main`.

To stop the check, turn off **Settings**, **Station**, **Check for new releases**. Off, it sends nothing at all, and the notice disappears as soon as you save.
