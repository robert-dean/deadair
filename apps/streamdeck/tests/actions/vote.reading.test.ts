...
describe('voteView', () => {
    ...
    it('writes its own opinion while the record’s rating is not known yet, so a first press is never a withdrawal', () => {
        expect(voteView(on, undefined, 'liked')).toEqual({ lit: false, title: '', press: { trackId: record.trackId, rating: 'liked' } });
    ...
    it('refuses a press on something with no row in the catalog to hold an opinion', () => {
        ...
        expect(voteView({ status, readAt: 1_000, stale: false }, undefined, 'liked')).toEqual({ lit: false, title: '' });
    });

    it('refuses a press with nothing on air', () => {
        expect(voteView({ status: stoodDown(), readAt: 1_000, stale: false }, { rating: 'liked' }, 'liked')).toEqual({ lit: false, title: '' });
    });

    it('goes dark on a stale reading, whatever it knew: the opinion may be about the record before this one', () => {
        const stale: Reading = { status: airing(), readAt: 1_000, failure: 'unreachable', stale: true };
        expect(voteView(stale, { rating: 'liked' }, 'liked')).toEqual({ lit: false, title: 'No station' });
    });

    it('says why there is no station to vote at', () => {
        expect(voteView({ failure: 'unconfigured', stale: false }, undefined, 'liked')).toEqual({ lit: false, title: 'Set up' });
    });
});
