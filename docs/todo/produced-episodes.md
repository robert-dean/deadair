# Deferred: produced episodes, and what a long generation does to the gates

**As of:** 2026-08-16, from asking what a podcast would do to `LlmGate` and `SpeechGate` a few hours
after both grew priority tiers.

Nothing here is built and nothing should be built from it yet. It exists because four of its
decisions are "do not do the obvious thing", and the obvious thing is what the current shape of
`LlmService.converse` actively invites. Getting them wrong is not a slow episode, it is an hour of
the station falling through to its deterministic floor with nothing saying why.
...
resumability gotchas that were paid for live. Read that first. This file is only what the CURRENT
tree does about it.
...
than a queue's — **a queue cannot cancel**, so a cancelled episode is marked terminally on its row
and every pass checks that row before it spends anything. The second v1 gotcha (no control entry
must count as still current) is a symptom of holding cancellation somewhere other than the thing
being cancelled, and does not arise if the row is the authority.
