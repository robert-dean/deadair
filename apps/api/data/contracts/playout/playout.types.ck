...
contract PlayoutPlaylistInput: { # The plugin playlist to load into the running order
    ...
    callins?: boolean # Whether somebody phones in during this broadcast, exactly as `PutOnAirInput.callins`. Absent takes the station's own `rotation.callins`, so a station that takes calls needs an explicit `false` here to air a playlist without them
}
...
contract PlayoutChartInput: { # The published chart to build the running order from
    ...
    callins?: boolean # Whether somebody phones in during this broadcast, exactly as `PutOnAirInput.callins`. Absent takes the station's own `rotation.callins`, so a station that takes calls needs an explicit `false` here to air a chart without them
}
