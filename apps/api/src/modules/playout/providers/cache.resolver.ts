...
export class CachedTrackResolver extends TrackResolver {
    ...
    async resolve(item: RundownItem): Promise<string | undefined> {
        ...
        try {
            const binding = await scope.get(TrackAudioRepository).findByBinding(item.pluginId, item.externalId);
            ...
            if (binding.dueForFetch) {
                await scope.get(JobBroker).send('playout.cache_track', { pluginId: item.pluginId, externalId: item.externalId });
            }
