...
| [break-removal.md](break-removal.md) | **The bug is fixed** (2026-08-12): deleting a talk break used to be forgotten by the next commit pass, because a spliced-out break left a gap the planner could not tell from one never planted into. A removed break is a `skipped` item now and its row is retired. Kept for the timed suppression the operator actually wanted, the move case, and what the fix cost in `committedThrough` |
