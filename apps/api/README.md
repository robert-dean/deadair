...
| `station.playlists`       | The station's own playlists: `GET /station-playlists`, `GET`/`PATCH`/`DELETE /station-playlists/:id`, `GET /station-playlists/:id/export` (a file another station can import), and `POST /station-playlists/import/preview` and `/import`, which run one planner so the preview is the decision |
