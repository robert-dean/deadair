...
Everything above keeps one directory, which is right until the library gets big. The station
downloads a record before it airs and keeps it, so the cached audio grows to roughly the size of
what you actually play — while the rest of that directory stays small and is the part that was
authored.

They can be split. Mount a second path at `/tracks` and the cache goes there instead:

```
-v /mnt/user/media/deadair-records:/tracks
```

On Unraid that is the **Record cache** field in the template; leave it blank and nothing changes.

The station decides by whether `/tracks` is really mounted, not by a setting you also have to
...
The two halves want different treatment. `/data` is small, authored and worth backing up, and
belongs on fast storage. `/tracks` is large, fetched, and worth no backup at all: delete it and
the station downloads what it needs again. `playout.trackCacheMaxBytes` puts a ceiling on it if
the disk it lives on has one.

## Backing it up

The data directory, and — if your database is elsewhere — a dump of it. Not the record cache. Between them they hold
