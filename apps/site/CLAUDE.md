...
**Five pages are copied in, never edited here.** `scripts/docs.sync.mjs` copies `deploy/README.md`,
`docs/licensing.md`, `CONTRIBUTING.md` (as `develop/contributing.md`) and
`packages/plugin-sdk/README.md` (as `plugin-development/contract.md`) into `docs/` on every `start`
and `build`, and `apps/android/PRIVACY.md` into `src/pages/privacy/android.md`,
...
an in-page anchor is left alone. That rewrite is why the SDK's README could join the others, since
it links into its own `src/`. `turbo.json` lists all five sources as inputs of this package's `build`,
...
**There is one sidebar per audience, and two of them are listed by hand.** `sidebars.ts` holds `run`
(operators), `build` (plugin and API authors) and `api`, and the navbar has one item per sidebar. The
hand-written sections are enumerated there rather than autogenerated, so renaming a page fails the
build instead of quietly moving it in the sidebar; a new hand-written page has to be added to the
list or it has no sidebar at all. `api-reference` is autogenerated from the directory, because it is
regenerated from the contracts and nobody here chooses what is in it. Adding a page to a section is
therefore two edits: the file, and the list.
