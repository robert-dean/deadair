...
export const Segment = z.strictObject({
    ...
    durationMs: z
        ...
        .optional()
        .describe('How long it runs. A display value: the player measures the audio itself'),
...
export const ScriptUsage = z.strictObject({
    ...
    outputTokens: z.preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0)).optional(),
    totalTokens: z.preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0)).optional(),
});
...
export const ScriptHistorySummaryQuery = z.strictObject({
    hours: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(1).max(168))
        .optional()
        .describe(
...
export const ScriptHistorySummaryRow = z.strictObject({
    ...
    written: z.preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0)),
    declined: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('A decline is the writer registry working: the model had nothing to say and the floor covered for it'),
    failed: z.preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0)),
});
...
export const SegmentScanResult = z.strictObject({
    scanned: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Audio files seen, whether or not they were already known'),
    imported: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Segments the station did not have before this pass'),
    skipped: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Files passed over: not audio it can serve, or unreadable'),
});
...
export const PadSetWrite = z.strictObject({
    ...
    label: z.string().min(1).max(200),
    position: z.preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0)).optional(),
});
...
export const PadScanResult = z.strictObject({
    scanned: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Audio files seen, whether or not anything changed'),
    imported: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Sounds the station did not have before'),
    replaced: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Slots whose file changed under them, which every script naming them now plays'),
    contested: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe(
    ...
    skipped: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .describe('Files passed over: not audio, unreadable, or named something no script could write'),
});
...
export const ScriptAttempt = z.strictObject({
    ...
    durationMs: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(0))
        .optional()
        .describe('How long the attempt took'),
...
export const ScriptHistorySummary = z.strictObject({
    hours: z
        .preprocess(v => (typeof v === 'string' && v.trim() !== '' ? Number(v) : v), z.number().int().min(1).max(168))
        .describe('The window actually counted, echoed so a console can label the numbers it draws'),
