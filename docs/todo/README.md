...
| [news-and-bulletins.md](news-and-bulletins.md) | **The source half is built** (2026-08-15): a `news` capability, an RSS plugin over an SDK feed parser, `NewsService`, and a `read_news` tool, so a model DJ has true things to say about the world. What is deferred is the news ON AIR — a `news` segment kind with a model writer over a headline-reading floor, which `clock.bands.ts` already parses `:30 news` for — plus the watcher that would post a breaking-news `BreakRequest`, which is deliberately a loop over any polled source rather than a news feature, and the conditional GET that becomes worth it the day something polls |
