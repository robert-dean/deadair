...
Do this before every upgrade, not only on a schedule. `latest` follows `main`, so pulling it is not a
numbered release with a changelog entry to weigh first: copy `/data` and dump the database before you
pull, and the backup taken just before is the way back if something in it does not fit your station.
