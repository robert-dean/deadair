...
contract CatalogPlaylist: {
    ...
    hidden?: boolean # An operator hid this playlist from this station, so pickers leave it out and the library sync does not read it. Absent when it is not hidden
}
