...
import { StatusPoller, type Playout, type Reading } from '../../src/station/status.poller.js';
...
describe('StatusPoller', () => {
    ...
    it('asks every two seconds while held, once for every key holding it', async () => {
        const { playout, getPlayoutStatus } = fakePlayout();
        const poller = new StatusPoller();
        poller.reconfigure(playout);
        poller.acquire();
    ...
    it('backs off while the station fails, doubling to thirty seconds, and recovers to two', async () => {
        ...
        expect(gaps.slice(0, 6)).toEqual([2_000, 4_000, 8_000, 16_000, 30_000, 30_000]);
        ...
        expect(times.slice(1).map((time, index) => time - times[index]!)).toEqual([2_000, 2_000]);
    });
