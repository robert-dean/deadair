 ...
 * generated from [TraceOutcome](file://./../../../../data/contracts/station/traces.types.ck#L10)
 */
 ...
 * generated from [TraceDecision](file://./../../../../data/contracts/station/traces.types.ck#L22)
 */
export const TraceDecision = z.strictObject({
    ...
    ms: z.coerce.number().int().min(0).describe('Wall clock, off the `job.run` span. Zero for a decision recorded before that span existed'),
    calls: z.coerce.number().int().min(0).describe('Everything it did, not counting the `job.run` that contains them'),
    failed: z.coerce.number().int().min(0).describe('How many of those did not produce what they were asked for'),
});
export type TraceDecision = z.infer<typeof TraceDecision>;

/**
 * Which slice of the kept window to read
 * generated from [TracesQuery](file://./../../../../data/contracts/station/traces.types.ck#L32)
 */
 ...
 * generated from [TraceSpan](file://./../../../../data/contracts/station/traces.types.ck#L12)
 */
export const TraceSpan = z.strictObject({
    ...
    ms: z.coerce.number().int().min(0).describe('How long it held, measured around the call rather than reported by it'),
...
/**
 * generated from [TracesPage](file://./../../../../data/contracts/station/traces.types.ck#L38)
 */
export const TracesPage = z.strictObject({
    decisions: z.array(TraceDecision).describe('Newest first'),
    total: z.coerce.number().int().min(0).describe('How many the window holds before `limit`, so a page can say it is showing a slice'),
    spans: z.coerce.number().int().min(0).describe('How many calls were read to answer, which is the honest cost of this page'),
});
export type TracesPage = z.infer<typeof TracesPage>;

/**
 * One decision, its calls, and the decisions on either side of it
 * generated from [TraceDetail](file://./../../../../data/contracts/station/traces.types.ck#L44)
 */
